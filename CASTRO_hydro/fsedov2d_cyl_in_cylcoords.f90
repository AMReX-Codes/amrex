! Process a cylindrical Sedov problem in cylindrical coordinates
! to produce rho, u, and p as a function of r, 
! for comparison to the analytic solution.

program fextract2d_cyl
     ...
     do j = 1, nboxes(pf, i)
        ...
        do ii = lbound(p,dim=1), ubound(p,dim=1)
              ...
              if ( any(imask(ii*r1:(ii+1)*r1-1) ) ) then
                 ...
                 dens_bin(index:index+(r1-1)) = p(ii,jmid,1,dens_comp)

                 vel_bin(index:index+(r1-1)) = &
                       abs(p(ii,1,1,xmom_comp)) / p(ii,jmid,1,dens_comp) 

                 pres_bin(index:index+(r1-1)) = p(ii,jmid,1,pres_comp) 
