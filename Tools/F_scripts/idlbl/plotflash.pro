pro plotflash, filename, var, xrange=xrange, yrange=yrange, $
...
; load the colortable -- we uses special colortables, the first 12
...
read_amr, filename, /double, DATA=unk, TREE=tree, PARAMETERS=params, STORED_VARS=stored_vars

help, unk
help, tree

temp_arr = create_variable(unk, var)
...
data = merge_amr(reform(temp_arr), TREE=tree, PARAMETERS=params, /double, XMERGE=x, YMERGE=y, $
