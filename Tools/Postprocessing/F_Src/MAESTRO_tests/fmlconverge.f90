! Take 2 plotfiles, one with 2x the resolution of the other and do
! a convergence test.  
!
!  -- The finer resolution calculation should have 2 levels.  
!  -- The coarser calculation should be 1 level
!
! We rely on the fact that in the fine calculation, the coarse level
! data will represent the average of the corresponding finer data, so
! we can simply compare the coarse data.
!
! The data on both files are put onto a uniform grid with the coarse
! resolution.  The --xmin, --xmax, ... runtime parameters can be used
! to pick the subdomain over which to compare.  
!
! The norm of the error (both absolute and relative) is returned.  
! Different norms can be choosen with the --norm runtime parameter.  
! norm = 0 is the inf-norm.
...
program fmlcompare
  ...
  if (pf_a%flevel == 1 .and. pf_b%flevel == 2) then
     a_is_fine = .false.
  elseif (pf_a%flevel == 2 .and. pf_b%flevel == 1) then
     a_is_fine = .true.
  else
     call bl_error("ERROR: we require one 1-level plotfile and one 2-level plotfile")
  endif


  ! make sure the corresponding coarse level dx's agree
  if (a_is_fine) then
     dx_a = plotfile_get_dx(pf_a, pf_a%flevel-1)
     dx_b = plotfile_get_dx(pf_b, pf_b%flevel)  
  else
     dx_a = plotfile_get_dx(pf_a, pf_a%flevel)
     dx_b = plotfile_get_dx(pf_b, pf_b%flevel-1)  
  endif
  ...
  if (a_is_fine) then
     flo_a = lwb(plotfile_get_pd_box(pf_a, pf_a%flevel-1))
     fhi_a = upb(plotfile_get_pd_box(pf_a, pf_a%flevel-1))
     ...
     flo_b = lwb(plotfile_get_pd_box(pf_b, pf_b%flevel-1))
     fhi_b = upb(plotfile_get_pd_box(pf_b, pf_b%flevel-1))
  endif
  ...
  if (a_is_fine) then
     print *, 'file 1 is multilevel -- using the coarser grid'
  else
     print *, 'file 2 is multilevel -- using the coarser grid'
  endif
  ...
  do n_a = 1, pf_a%nvars
     ...
     case (2)
        ...
        if (a_is_fine) then
           comp(1) = n_a
           call blow_out_to_sub_fab(fab_a_2d, tlo, thi, &
                                    pf_a, comp, pf_a%flevel-1)

           comp(1) = n_b
           call blow_out_to_sub_fab(fab_b_2d, tlo, thi, &
                                    pf_b, comp, pf_b%flevel)
        else
           comp(1) = n_a
           call blow_out_to_sub_fab(fab_a_2d, tlo, thi, &
                                    pf_a, comp, pf_a%flevel)

           comp(1) = n_b
           call blow_out_to_sub_fab(fab_b_2d, tlo, thi, &
                                    pf_b, comp, pf_b%flevel-1)
        endif
     ...
     case (3)
        ...
        if (a_is_fine) then
           comp(1) = n_a
           call blow_out_to_sub_fab(fab_a_3d, tlo, thi, &
                                    pf_a, comp, pf_a%flevel-1)

           comp(1) = n_b
           call blow_out_to_sub_fab(fab_b_3d, tlo, thi, &
                                    pf_b, comp, pf_b%flevel)
        else
           comp(1) = n_a
           call blow_out_to_sub_fab(fab_a_3d, tlo, thi, &
