...
function (set_cuda_architectures _cuda_archs)
   ...
   set(_cmake_archs)
   set(_all_numeric TRUE)
   set(_has_numeric_sm10_or_newer FALSE)

   foreach (_item IN LISTS _archs)
      set(_arch_no_suffix "${_item}")
      set(_arch_suffix "")
      if (_arch_no_suffix MATCHES "-real$")
         string(REGEX REPLACE "-real$" "" _arch_no_suffix "${_arch_no_suffix}")
         set(_arch_suffix "-real")
      elseif (_arch_no_suffix MATCHES "-virtual$")
         string(REGEX REPLACE "-virtual$" "" _arch_no_suffix "${_arch_no_suffix}")
         set(_arch_suffix "-virtual")
      endif ()

      if (_arch_no_suffix MATCHES "^([0-9]+)\\.([0-9]+)$")
         set(_arch_major "${CMAKE_MATCH_1}")
         set(_arch_minor "${CMAKE_MATCH_2}")
         list(APPEND _cmake_archs "${_arch_major}${_arch_minor}${_arch_suffix}")
         if (_arch_major GREATER_EQUAL 10)
            set(_has_numeric_sm10_or_newer TRUE)
         endif ()
      elseif (_arch_no_suffix MATCHES "^[0-9]+$")
         list(APPEND _cmake_archs "${_arch_no_suffix}${_arch_suffix}")
         if (_arch_no_suffix GREATER_EQUAL 100)
            math(EXPR _arch_major "${_arch_no_suffix}/10" OUTPUT_FORMAT DECIMAL)
            if (_arch_major GREATER_EQUAL 10)
               set(_has_numeric_sm10_or_newer TRUE)
            endif ()
         endif ()
      else ()
         set(_all_numeric FALSE)
      endif ()
   endforeach ()

   # The legacy cuda_select_nvcc_arch_flags helper does not understand SM 10+.
   # For modern CMake target properties, explicit numeric archs can be used directly.
   if (_all_numeric AND _has_numeric_sm10_or_newer)
      set(AMREX_CUDA_ARCHS ${_cmake_archs} CACHE INTERNAL "CUDA archs AMReX is built for")
      return()
   endif ()
