...
void main_main()
{
        ...
        if (ilev < max_level) {
            ...
            for (MFIter mfi(pltmf); mfi.isValid(); ++mfi) {
                ...
                for (int idir = ndir_begin; idir < ndir_end; ++idir) {
                    ...
                    if (ibox.ok()) {
                        ...
                        rrslice[idir] = 1;
                        amrex::For(ibox, [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
                        {
            ...
            for (MFIter mfi(pltmf); mfi.isValid(); ++mfi) {
                ...
                for (int idir = ndir_begin; idir < ndir_end; ++idir) {
                    ...
                    if (ibox.ok()) {
                        const auto& data = datamf[idir].array(0); // there is only one box
                        amrex::ParallelFor(ibox, [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
                        {
    ...
    for (int idir = ndir_begin; idir < ndir_end; ++idir) {
        ...
        amrex::ParallelFor(finebox[idir], [=] AMREX_GPU_HOST_DEVICE (int i, int j, int k)
        {
