...
def doCVSUpdate(topDir, root, outDir):
   ...
   command_for_shell = "cvs update %s >& cvs.%s.out" % (root, root)
   os.system('bash -c "'+command_for_shell+'"')
   print "\n"
...
def test(argv):
    ...
    options:
       ...
       --cvs_update
          do the cvs update and then run the suite.
    ...
    os.system("make COMP=%s realclean" % (FCOMP))

    command_for_shell = "make -j 4 programs=fcompare NDEBUG=t MPI= COMP=%s  >& fcompare.make.out" % (FCOMP)
    os.system('bash -c "'+command_for_shell+'"')
    ...
    command_for_shell = "make -j 4 programs=fsnapshot2d NDEBUG=t MPI= COMP=%s  >& fsnapshot2d.make.out" % (FCOMP)
    os.system('bash -c "'+command_for_shell+'"')
    vis2dExecutable = getRecentFileName(compareToolDir,"fsnapshot2d",".exe")
    
    command_for_shell = "make -j 4 programs=fsnapshot3d NDEBUG=t MPI= COMP=%s  >& fsnapshot3d.make.out" % (FCOMP)
    os.system('bash -c "'+command_for_shell+'"')
    ...
    for test in tests:
	    ...
	    if (useMPI):
	       ...
	       command_for_shell = "make -j 4 DIM=%d USE_MPI=TRUE COMP=%s FCOMP=%s executable=%s  >& %s/%s.make.out" % \
                   (dim, COMP, FCOMP, executable, outputDir, test)
               os.system('bash -c "'+command_for_shell+'"')
            else:
	       executable = "%s%dd.ex" % (suiteName, dim)
	       command_for_shell = "make -j 4 DIM=%d USE_MPI=false COMP=%s FCOMP=%s executable=%s  >& %s/%s.make.out" % \
                   (dim, COMP, FCOMP, executable, outputDir, test)
               os.system('bash -c "'+command_for_shell+'"')
	       
            
        elif (sourceTree == "fParallel"):
            command_for_shell = "make -j 4 MPI= NDEBUG=t COMP=%s >& %s/%s.make.out" % (FCOMP, outputDir, test)
            os.system('bash -c "'+command_for_shell+'"')
	...
	useMPI = getParam(test + ".useMPI")
	    ...
	    if (useMPI):
	       ...
	       print testRunCommand
               ...
               os.system('bash -c "'+testRunCommand+'"')
	       
            else:
               command_for_shell = "./%s %s amr.plot_file=%s_plt amr.check_file=%s_chk >&  %s.run.out" % (executable, inputsFile, test, test, test)
               os.system('bash -c "'+command_for_shell+'"')
            ...
            if (restart):
               command_for_shell = "./%s %s --plot_base_name %s_plt --check_base_name %s_chk >& %s.run.out" % (executable, inputsFile, test, test, test)
               os.system('bash -c "'+command_for_shell+'"')
            else:
               command_for_shell = "./%s %s --plot_base_name %s_plt --check_base_name %s_chk --chk_int 0 >& %s.run.out" % \
                   (executable, inputsFile, test, test, test)
               os.system('bash -c "'+command_for_shell+'"')
	   ...
	   restartFile = "%s_chk%5.5d" % (test, restartFileNum)
           ...
           if (sourceTree == "Parallel"):
              command_for_shell = "./%s %s amr.plot_file=%s_plt amr.check_file=%s_chk amr.restart=%s >>  %s.run.out 2>&1" % \
                                  (executable, inputsFile, test, test, restartFile, test)
              os.system('bash -c "'+command_for_shell+'"')
           elif (sourceTree == "fParallel"):
              command_for_shell = "./%s %s --plot_base_name %s_plt --check_base_name %s_chk --restart %d >> %s.run.out 2>&1" % \
        ...
        for file in os.listdir(outputDir):
            if (os.path.isdir(file) and 
                ...
                command_for_shell = "tar czf %s.tgz %s >& %s.tar.out" % (file, file, file)
                err = os.system('bash -c "'+command_for_shell+'"')
                if (err == 0):
                    shutil.rmtree(file)
