...
def set_initial_conditions(ncells, domain_min, domain_max, injected_plasma_density, injected_plasma_ppc, plasma_min, plasma_max):
    '''
...
def inject_plasma(num_shift, direction, injected_plasma_density, injected_plasma_ppc, plasma_min, plasma_max):
    '''
    ...
    nplasma_cells = ((plasma_max_inject - plasma_min_inject)/dx + 0.5).astype('l') + 1
    iplasma_min = ((plasma_min_inject - domain_min)/dx).astype('l')*dx + domain_min

    weight = injected_plasma_density * dx[0]*dx[1]*dx[2] / injected_plasma_ppc
    ...
    plasma_locs = np.logical_and(xp >= plasma_min_inject[0], xp < plasma_max_inject[0])
    plasma_locs = np.logical_and(plasma_locs, yp >= plasma_min_inject[1])
    plasma_locs = np.logical_and(plasma_locs, zp >= plasma_min_inject[2])
    plasma_locs = np.logical_and(plasma_locs, yp  <  plasma_max_inject[1])
    plasma_locs = np.logical_and(plasma_locs, zp  <  plasma_max_inject[2])
    ...
    add_particles(0, xp[lo:hi], yp[lo:hi], zp[lo:hi],
                  ...
                  wp[lo:hi], unique_particles=True)
...
set_initial_conditions(ncells, domain_min, domain_max, injected_plasma_density, injected_plasma_ppc, plasma_min, plasma_max)
...
for i in range(1, max_step + 1):
    ...
    if (num_shift != 0):
        inject_plasma(num_shift, direction, injected_plasma_density, injected_plasma_ppc, plasma_min, plasma_max)
