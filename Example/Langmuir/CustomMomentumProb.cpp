...
/* This momentum distribution corresponds to a 3D plasma wave.
   ...
   $$ \phi = \epsilon \,\frac{m_e c^2}{q_e}\cos(k_x x)\cos(k_y y)\cos(k_z z) \sin(\omega_p t) $$
   ...
   $$ v_x/c = -\epsilon \,\frac{m_e q c k_x}{m q_e \omega_p}\sin(k_x x)\cos(k_y y)\cos(k_z z)\cos(\omega_p t) $$
   $$ v_y/c = -\epsilon \,\frac{m_e q c k_y}{m q_e \omega_p}\cos(k_x x)\sin(k_y y)\cos(k_z z)\cos(\omega_p t) $$
   $$ v_z/c = -\epsilon \,\frac{m_e q c k_z}{m q_e \omega_p}\cos(k_x x)\cos(k_y y)\sin(k_z z)\cos(\omega_p t) $$
*/
...
void CustomMomentumDistribution::getMomentum(vec3& u, Real x, Real y, Real z) {
  ...
  u[0] = -epsilon * c*kx/wp * std::sin(kx*x) * std::cos(ky*y) * std::cos(kz*z);
  u[1] = -epsilon * c*kx/wp * std::cos(kx*x) * std::sin(ky*y) * std::cos(kz*z);
  u[2] = -epsilon * c*kx/wp * std::cos(kx*x) * std::cos(ky*y) * std::sin(kz*z);
}
