...
multifabs that may include a different number of ghost cells.  The fillpatch operation fills all cells,
valid and ghost, from actual valid data at that level, neighboring grids at the same level, domain
boundary conditions (for examples that have non-periodic
boundary conditions), or space-time interpolated data from the next-coarser level.
...
A {\tt FillPatchUtil} uses an {\tt Interpolator}.
...
Figure \ref{fig:AmrAdvection_AmrCore_flowchart} shows a flowchart
...
\end{itemize}
\item {\tt Advection\_AmrCore/Src} Source code specific to this example.  The subdirectories {\tt Src\_2d}
...
contain an implementation.  In {\tt AmrAdv.cpp} the {\tt ErrorEst} function is essentially an
...
This example has to functions, {\tt AmrAdv::FillPatch} and {\tt AmrAdv::CoarseFillPatch},
...
actual valid data at that level, neighboring grids at the same level, domain boundary conditions 
(for examples that have non-periodic
boundary conditions), or space-time interpolated data from the next-coarser level.
\begin{lstlisting}[language=cpp]
