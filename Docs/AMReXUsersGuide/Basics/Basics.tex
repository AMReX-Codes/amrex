...
\begin{lstlisting}[language=cpp]
  for (MFIter mfi(mf); mfi.isValid(); ++mfi) // Loop over grids
  {
      ...
      // and is thus larger than or equal to "vbox".
      const Box& abox = fab.box();

      // We can now pass the information to a function that does
      // work on the region specified by vbox of the data pointed
      ...
      f1(vbox.loVect(), vbox.hiVect(), a, abox.loVect(), abox.hiVect());
  }
\end{lstlisting}
Here function {\tt f1} is usually a Fortran subroutine with ISO C
binding interface (see Section~\ref{sec:basics:fortran} for more
details).  In the example above, {\tt MultiFab} is assumed to have a
single component.  If it has multiple component, we can call {\tt
int nc = mf.nComp()} to get the number of components and pass it to
the kernel function.  There is only one {\tt MultiFab} in the example
...
C binding interface (see Section~\ref{sec:basics:fortran} for more
details).


\subsection{Logical Tiling}

\subsection{MFIter with Tiling}

% mfs with tiling

% all kinds of tile boxes
