\documentclass[11pt]{book} 

\tolerance=600

\usepackage{color,longtable,epsfig,epsf,amsmath}

% Margins
\usepackage{fullpage}

\newcommand{\BoxArray}{{\tt BoxArray}}
\newcommand{\BoxLib}{{\tt BoxLib}}
\newcommand{\BoxType}{{\tt Box}}
\newcommand{\IntVect}{{\tt IntVect}}
\newcommand{\Fab}{{\tt Fab}}
\newcommand{\FArrayBox}{{\tt FArrayBox}}
\newcommand{\MultiFab}{{\tt MultiFab}}
\newcommand{\layout}{{\tt layout}}

\setlength{\marginparwidth}{0.8in}
\newcommand{\MarginPar}[1]{\marginpar{%
\vskip-\baselineskip %raise the marginpar a bit
\raggedright\tiny\sffamily
\hrule\smallskip{\color{red}#1}\par\smallskip\hrule}}

\newcommand{\sfrac}[2]{\mathchoice
  {\kern0em\raise.5ex\hbox{\the\scriptfont0 #1}\kern-.15em/
   \kern-.15em\lower.25ex\hbox{\the\scriptfont0 #2}}
  {\kern0em\raise.5ex\hbox{\the\scriptfont0 #1}\kern-.15em/
   \kern-.15em\lower.25ex\hbox{\the\scriptfont0 #2}}
  {\kern0em\raise.5ex\hbox{\the\scriptscriptfont0 #1}\kern-.2em/
   \kern-.15em\lower.25ex\hbox{\the\scriptscriptfont0 #2}}
  {#1\!/#2}}

\newcommand{\myhalf}{\sfrac{1}{2}}

% special colors and definitions for framing source code listings
\usepackage{listings}

\definecolor{gray}{rgb}       {0.8,0.8,0.8}
\definecolor{light-blue}{rgb} {0.8,0.8,1.0}
\definecolor{light-green}{rgb}{0.8,1.0,0.8}
\definecolor{light-red}{rgb}  {1.0,0.9,0.9}

\lstset{
  basicstyle=\small\ttfamily,%
  frame=shadowbox,%
  rulesepcolor=\color{gray},%
  backgroundcolor=\color{white}%
}

\renewcommand{\floatpagefraction}{1.0}
\renewcommand{\topfraction}{1.0}
\renewcommand{\bottomfraction}{1.0}
\renewcommand{\textfraction}{0.0}

\title{\Huge{\bf \BoxLib\ User's Guide}\\
       \vspace{0.5in}
       \Large{Center for Comutational Sciences and Engineering}\\
       \Large{Lawrence Berkeley National Laboratory}\\
       \Large{\tt https://ccse.lbl.gov}
       \date{April 20, 2012}
\begin{figure}[b]
\centering
\includegraphics[width=2in]{./Introduction/varden1}
\includegraphics[width=2in]{./Introduction/varden2}\\
\includegraphics[width=2in]{./Introduction/varden3}
\includegraphics[width=2in]{./Introduction/varden4}
\end{figure}
}

% don't put a header on blank pages, see
% http://www.latex-community.org/forum/viewtopic.php?f=4&p=51559
% change ``plain'' to ``empty'' to eliminate the page number
\makeatletter
\renewcommand*\cleardoublepage{\clearpage\if@twoside
\ifodd\c@page\else
\hbox{}
\thispagestyle{empty}
\newpage
\if@twocolumn\hbox{}\newpage\fi\fi\fi}
\makeatother



\begin{document}



\frontmatter

\maketitle

\cleardoublepage

\setcounter{tocdepth}{1}
\addcontentsline{toc}{part}{Table of Contents}
\tableofcontents

\cleardoublepage

%\addcontentsline{toc}{part}{List of Figures}
%\listoffigures

\cleardoublepage

\chapter*{Preface}
\addcontentsline{toc}{part}{Preface}
\input Preface/Preface

\mainmatter

\chapter{Introduction}\label{Chap:Introduction}
\input Introduction/Introduction

\chapter{Getting Started}\label{Chap:Getting Started}
\input GettingStarted/GettingStarted

\chapter{Advanced Topics With Fortran90 \BoxLib}\label{Chap:Advanced Topics}
\input AdvancedTopics/AdvancedTopics

\end{document}
