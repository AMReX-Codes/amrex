...
Note that when a C++ application is compiled and linked,
...
This is unlike the Fortran90 data structures in which we build
...
\begin{lstlisting}[backgroundcolor=\color{DarkerAntiqueWhite3}]
IntVect iv(3,5);
\end{lstlisting}
In Fortran90, we don't use \IntVect\ s, but instead use standard
arrays of integers:
\begin{lstlisting}[backgroundcolor=\color{AntiqueWhite3}]
integer :: iv(2)
iv(1) = 3
iv(2) = 5
\end{lstlisting}
...
{\tt IntVect lo} and {\tt IntVect hi}.  In C++, a \BoxType\ also
contains an {\tt IndexType} (cell-centered, face-centered, or nodal) for each
dimension.  In Fortran90, a \BoxType\ also contains the dimensionality 
of the \BoxType.  To build a \BoxType\ in Fortran90 use:
\begin{lstlisting}[backgroundcolor=\color{AntiqueWhite3}]
type(box) :: bx
...
refinement.  In C++, a \MultiFab\ is defined using a \BoxArray,
number of components, and number of ghost cells that each \Fab\
will have.  In Fortran90, a \MultiFab\ is defined using a \layout,
...
In Fortran90, use:
\begin{lstlisting}[=AntiqueWhite3]
