"""
This script rewrites the filelist.rst file autogenerated by breathe to organize the files in the documentation's API into the same directory structure as their counterparts in the ../../Src directory.

We are not going to include cpp files - doxygen assumes that functions are documented in the header files, so that's what we'll assume here as well.

TODO: check if doxygen has generated the corresponding files and only print their names if so
...
def generate_filelist(rootdir, outfile, output_data, subdir_prefix=""):
   ...
   """.format(subdir_prefix + subdir)
            ...
            subdir_file_name = "source/{}_files.rst".format(subdir_prefix + subdir)
...
.. toctree::
   ...
   """
                generate_filelist(os.path.join(rootdir, subdir), subdir_file,
                                  subdir_output_data, subdir_prefix=subdir_prefix+subdir+'_')
