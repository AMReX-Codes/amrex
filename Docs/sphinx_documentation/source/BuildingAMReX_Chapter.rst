...
the section on :ref:`sec:build:make`.  

The second approach is to build install AMReX as a library using GNU make (:ref:`sec:build:lib`); 
an application code then uses its own build system and links to AMReX as an external library.   
It is also possible to install a lagged version of the master branch of AMReX using spack 
(https://spack.io/).  
