...
In a similar fashion to the ``SWFFT`` tutorial:

To build the code, type 'make' in ``amrex/Tutorials/SWFFT_simple``.  This
will include code from ``amrex/Src/Extern/SWFFT_simple`` and you will need to
link to the FFT solvers themselves (on NERSC's Cori machine, for example,
you would need to "module load fft")
...
The :ref:`section:swfft_tutorial:swfft` tutorial provides an example of solving a Poisson equation using a discrete spectral method, in which a forward and reverse FFT of a :cpp:`MultiFab` are computed.
