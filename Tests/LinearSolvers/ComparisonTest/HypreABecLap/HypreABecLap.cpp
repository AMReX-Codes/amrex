...
TransverseInterpolant(AuxVarBox& cintrp, const Mask& msk,
                      ...
                      BL_D_DECL(const IntVect& rat, const IntVect& vj1, const IntVect& vk1),
                      BL_D_DECL(const IntVect& ve,  const IntVect& vjr, const IntVect& vkr),
                      BL_D_DECL(int idir, int jdir, int kdir),
                      int clevel)
{
    ...
    for (OrientationIter oitr; oitr; ++oitr) {
      ...
      for (int i = cintrp[level].firstLocal(); cintrp[level].isValid(i);
        ...
        TransverseInterpolant(cintrp[level](ori)[i], msk, reg, creg,
                              BL_D_DECL(rat,  vj1,  vk1),
                              BL_D_DECL(ve,   vjr,  vkr),
                              BL_D_DECL(idir, jdir, kdir),
                              level-1);
