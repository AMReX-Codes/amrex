...
/***************/
               ...
               const EBLevelGrid& a_eblgCoar)
  {
    ...
    FabArray<EBCellFAB> phiCoarCalc(a_eblgCoar.getDBL(), a_eblgCoar.getDM(), nvar, nghost, MFInfo(), factCoar);
    ...
    //use kappa weighting = true because I have not kappa weighted the data
    bool useKappa = true;
    EBCoarseAverage aveOp(a_eblgFine, a_eblgCoar, nref, nghost, useKappa);


    aveOp.average(phiCoarCalc, phiFineExac, 0, 0, nvar);
    ...
    for(MFIter mfi(a_eblgCoar.getDBL(), a_eblgCoar.getDM()); mfi.isValid(); ++mfi)
    {
      ...
      {
        const VolIndex& vof = vofit();
        Real diff = std::abs(phiCoarCalc[mfi](vof,0)-phiCoarExac[mfi](vof,0));
...
/**********/
  ...
  int coarseAveTest()
  {
    ...
    Vector<EBLevelGrid> veblgCoar, veblgMedi, veblgFine;
    ...
    getError(errorMedi, eblgFine, eblgMedi);
    getError(errorCoar, eblgMedi, eblgCoar);

    EBLevelDataOps::compareError(errorMedi, errorCoar, eblgMedi, eblgCoar);
    return 0;
  }
}
/***************/
