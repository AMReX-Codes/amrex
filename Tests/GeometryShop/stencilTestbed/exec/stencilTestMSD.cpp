    ...
    for (MFIter mfi(srcMF,tilesize); mfi.isValid(); ++mfi)
    {
      ...
      for (int n=0; n<a_nodes.size(); ++n)
      {
        ...
        if (tbx.contains(iv))
        {
          ...
          RealVect normal(BL_D_DECL(0,0,0));
          for (int idir=0; idir<SpaceDim; ++idir)
          {
            for (SideIterator sit; sit.ok(); ++sit)
            {
      ...
      if ( ifs.is_open() )
      {
        ...
        for (int i=0; i<faceData[idir].size(); ++i)
        {
          ...
          for (int niv=0; niv<num_ivs; ++niv)
          {
            ...
            IntVect iv(BL_D_DECL(idxs[0],idxs[1],idxs[2]));
            ...
            RealVect cent(BL_D_DECL(c[0],c[1],c[2]));
      ...
      if (num_tiles > 0)
      {
        ...
        for (int i=0; i<bndryData.size(); ++i)
        {
          ...
          for (int niv=0; niv<num_ivs; ++niv)
          {
            ...
            Real c[SpaceDim];
            ifs.read((char*)(c), SpaceDim * sizeof(Real));
            RealVect normal(BL_D_DECL(c[0],c[1],c[2]));

            ifs.read((char*)(c), SpaceDim * sizeof(Real));
            RealVect centroid(BL_D_DECL(c[0],c[1],c[2]));
