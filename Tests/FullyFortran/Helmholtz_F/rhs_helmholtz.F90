  ...
  subroutine actual_init_abeclapcian (lo, hi, glo, ghi, rhs, rlo, rhi, exact, elo, ehi, &
    ...
    do k = glo(3), ghi(3)
       ...
       do j = glo(2), ghi(2)
          ...
          do i = glo(1), ghi(1)
             ...
             beta(i,j,k) = x*y*z!1.0d0!(sigma-1.d0)/2.d0*tanh(theta*(r-0.25d0)) + (sigma+1.d0)/2.d0
          end do
       end do
    end do
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
	     ...
	     exact(i,j,k) = cos(pi*x)*cos(pi*y)*cos(pi*z)
		...
		!rhs(i,j,k)=cos(pi*x)+2.0d0*pi**2*x*cos(pi*x)
		!rhs(i,j,k)=2.0d0*pi**2*x*cos(pi*x)
		!rhs(i,j,k)=cos(pi*x)+x*pi**2*cos(pi*x)
		!rhs(i,j,k)=cos(pi*x)+2.0*pi*(sin(pi*x)+x*pi*cos(pi*x))

		rhs(i,j,k)=a*alpha(i,j,k)*cos(pi*x)*cos(pi*y)*cos(pi*z)+b*pi*(y*z*cos(pi*y)*cos(pi*z)*(x*pi*cos(pi*x)+sin(pi*x))+ &
							         x*z*cos(pi*x)*cos(pi*z)*(y*pi*cos(pi*y)+sin(pi*y))+ &
								 x*y*cos(pi*x)*cos(pi*y)*(z*pi*cos(pi*z)+sin(pi*z))) 
          end do
       end do
    end do
