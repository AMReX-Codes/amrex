...
#include <WMLES.H>
#include <WMLES_K.H>
#include <WMLES_tagging.H>
#include <WMLES_parm.H>
#include <wmles_prob.H>
...
int       WMLES::verbose = 0;
IntVect   WMLES::hydro_tile_size {AMREX_D_DECL(1024,16,16)};
Real      WMLES::cfl       = 0.3;
int       WMLES::do_reflux = 1;
int       WMLES::refine_cutcells          = 1;
int       WMLES::refine_max_dengrad_lev   = -1;
Real      WMLES::refine_dengrad           = 1.0e10;
Vector<RealBox> WMLES::refine_boxes;
RealBox*  WMLES::dp_refine_boxes;

bool      WMLES::do_visc        = true;  // diffusion is on by default
bool      WMLES::use_const_visc = false; // diffusion does not use constant viscosity by default

int       WMLES::plm_iorder     = 2;     // [1,2] 1: slopes are zero'd,  2: second order slopes
Real      WMLES::plm_theta      = 2.0;   // [1,2] 1: minmod; 2: van Leer's MC
Real      WMLES::gravity        = 0.0;

int       WMLES::eb_weights_type = 0;   // [0,1,2,3] 0: weights are all 1
int       WMLES::do_reredistribution = 1;

WMLES::WMLES () = default;

WMLES::WMLES (Amr&            papa,
          int             lev,
...
void
WMLES::init (AmrLevel& old)
{
    auto& oldlev = dynamic_cast<WMLES&>(old);
...
WMLES::initData ()
{
    ...
    {
        wmles_initdata(i, j, k, sma[box_no], geomdata, *lparm, *lprobparm);
    });
...
void
WMLES::computeInitialDt (int                    finest_level,
                       int                    /*sub_cycle*/,
                       Vector<int>&           n_cycle,
...
void
WMLES::computeNewDt (int                    finest_level,
                   int                    /*sub_cycle*/,
                   Vector<int>&           n_cycle,
                   const Vector<IntVect>& /*ref_ratio*/,
                   Vector<Real>&          dt_min,
...
void
WMLES::post_regrid (int /*lbase*/, int /*new_finest*/)
{
}

void
WMLES::post_timestep (int /*iteration*/)
{
    if (do_reflux && level < parent->finestLevel()) {
        WMLES& fine_level = getLevel(level+1);
...
void
WMLES::postCoarseTimeStep (Real /*time*/)
{
...
void
WMLES::errorEst (TagBoxArray& tags, int, int, Real /*time*/, int, int)
{
    BL_PROFILE("WMLES::errorEst()");
...
//        const char clearval = TagBox::CLEAR;
        ...
        [=] AMREX_GPU_DEVICE (int box_no, int i, int j, int k) noexcept
        {
            wmles_tag_denerror(i, j, k, tagma[box_no], rhoma[box_no], dengrad_threshold, tagval);
        });
        Gpu::streamSynchronize();
    }
}
...
WMLES::read_params ()
{
    ...
    if (eb_weights_type < 0 || eb_weights_type > 3) {
        amrex::Abort("WMLES: eb_weights_type must be 0,1,2 or 3");
    }

    pp.query("do_reredistribution", do_reredistribution);
    if (do_reredistribution != 0 && do_reredistribution != 1) {
        amrex::Abort("WMLES: do_reredistibution must be 0 or 1");
    }
    ...
    for (MFIter mfi(S,false); mfi.isValid(); ++mfi)
    {
        ...
        if (flag.getType(bx) != FabType::covered)
        {
          reduce_op.eval(bx, reduce_data, [=]
             ...
             {
                 return wmles_estdt(i,j,k,s_arr,dx,*lparm);
             });
        }
    } // mfi
...
Real
WMLES::initialTimeStep ()
{
    return estTimeStep();
}

void
WMLES::computeTemp (MultiFab& State, int ng)
{
    BL_PROFILE("WMLES::computeTemp()");
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(State,true); mfi.isValid(); ++mfi)
    {
        ...
        if (flag.getType(bx) != FabType::covered) {
            ...
            {
                wmles_compute_temperature(i, j, k, s_arr, *lparm);
            });
        }
    }
}
