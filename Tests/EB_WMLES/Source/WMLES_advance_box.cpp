...
void
WMLES::compute_dSdt_box (const Box& bx,
                       Array4<Real const>& sfab,
                       Array4<Real      >& dsdtfab,
                       const std::array<FArrayBox*, AMREX_SPACEDIM>& flux)
{
    BL_PROFILE("WMLES::compute_dSdt_box()");
    ...
    if (do_visc == 1)
    {
       ...
       if(use_const_visc == 1 ) {
          ...
          [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
          {
              wmles_diffcoef(i, j, k, q, coefs, dxinv, *lparm);
          });
       }
    }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_slope_x(i, j, k, slope, q, l_plm_iorder, l_plm_theta);
    });
    const Box& xflxbx = amrex::surroundingNodes(bx,cdir);
    amrex::ParallelFor(xflxbx,
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_riemann_x(i, j, k, fxfab, slope, q, *lparm);
    ...
    if (do_visc == 1)
    {
       ...
       [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
       {
           wmles_diff_x(i, j, k, q, coefs, dxinv, fxfab);
       });
    }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_slope_y(i, j, k, slope, q, l_plm_iorder, l_plm_theta);
    });
    const Box& yflxbx = amrex::surroundingNodes(bx,cdir);
    amrex::ParallelFor(yflxbx,
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_riemann_y(i, j, k, fyfab, slope, q, *lparm);
    ...
    if(do_visc == 1)
    {
       ...
       [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
       {
           wmles_diff_y(i, j, k, q, coefs, dxinv, fyfab);
       });
    }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_slope_z(i, j, k, slope, q, l_plm_iorder, l_plm_theta);
    });
    const Box& zflxbx = amrex::surroundingNodes(bx,cdir);
    amrex::ParallelFor(zflxbx,
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_riemann_z(i, j, k, fzfab, slope, q, *lparm);
    ...
    if(do_visc == 1)
    {
       ...
       [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
       {
           wmles_diff_z(i, j, k, q, coefs, dxinv, fzfab);
       });
    }
#endif
    ...
    {
        wmles_flux_to_dudt(i, j, k, n, dsdtfab, AMREX_D_DECL(fxfab,fyfab,fzfab), dxinv);
    });
