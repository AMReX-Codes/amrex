...
void
WMLES::compute_dSdt_box_eb (const Box& bx,
                          ...
                          Real dt)
{
    BL_PROFILE("WMLES::compute_dSdt_box_eb()");
    ...
    {
        wmles_ctoprim(i, j, k, s_arr, q, *lparm);
    });
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_slope_eb_x(i, j, k, slope, q, flag, l_plm_iorder, l_plm_theta);
    });
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_riemann_x(i, j, k, fx_arr, slope, q, *lparm);
    ...
    if (do_visc == 1)
    {
        ...
        [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            wmles_diff_eb_x(i, j, k, q, coefs, flag, dxinv, weights, fx_arr);
        });
    }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_slope_eb_y(i, j, k, slope, q, flag, l_plm_iorder, l_plm_theta);
    });
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_riemann_y(i, j, k, fy_arr, slope, q, *lparm);
    ...
    if(do_visc == 1)
    {
       ...
       [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
       {
           wmles_diff_eb_y(i, j, k, q, coefs, flag, dxinv, weights, fy_arr);
       });
    }
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_slope_eb_z(i, j, k, slope, q, flag, l_plm_iorder, l_plm_theta);
    });
    ...
    [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
    {
        wmles_riemann_z(i, j, k, fz_arr, slope, q, *lparm);
    ...
    if(do_visc == 1)
    {
       ...
       [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
       {
           wmles_diff_eb_z(i, j, k, q, coefs, flag, dxinv, weights, fz_arr);
       });
    }
#endif
