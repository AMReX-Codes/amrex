...
contains
  ...
  subroutine advection_dudt(lo, hi, &
    ...
    use compute_flux_module, only : compute_flux_mol_2d

    integer, intent(in) :: lo(2), hi(2)
    real(amrex_real), intent(in) :: dx(2)
    integer, intent(in) :: ui_lo(2), ui_hi(2)
    integer, intent(in) :: du_lo(2), du_hi(2)
    ...
    real(amrex_real), intent(  out) :: dudt(du_lo(1):du_hi(1),du_lo(2):du_hi(2))
    ...
    call compute_flux_mol_2d(lo, hi, &
                             uin, ui_lo, ui_hi, &
                             vx, vx_lo, vx_hi, &
                             vy, vy_lo, vy_hi, &
                             flxx, fx_lo, fx_hi, &
    ...
    do    j = lo(2),hi(2)
       do i = lo(1),hi(1)
          dudt(i,j) = (flxx(i,j) - flxx(i+1,j)) * dxinv(1) &
               +      (flxy(i,j) - flxy(i,j+1)) * dxinv(2)
       enddo
    enddo
