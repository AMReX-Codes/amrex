...
void main_main ()
{
    ...
    amrex::Gpu::ManagedVector< Array4<Real> > arrs(nwrites);
...
//    amrex::ResetRandomSeed(33344455666);
    for (MFIter mfi(mfs[0]); mfi.isValid(); ++mfi) {
        ...
        Array4<Real>* array_ptr = arrs.dataPtr(); 

        amrex::ParallelFor(bx, [=] AMREX_GPU_DEVICE (int i, int j, int k) noexcept
        {
            for (int m = 0; m < nwrites; ++m) {
                array_ptr[m](i,j,k) = amrex::Random();
            }
        });
...
// ***************************************************************
    ...
    amrex::Print() << " Async-MPI API" << std::endl;
    {
        BL_PROFILE_REGION("vismf-async-mpi-api");
        for (int m = 0; m < nwrites; ++m) {
            VisMF::WriteAsyncMPIAPI(mfs[m], std::string("vismfdata/api-" + std::to_string(m)));
        }
        {
            BL_PROFILE_VAR("vismf-async-mpi-ibarrier-waitall-work", blp2);
            for (int m = 0; m < nwrites; ++m) {
