...
class TestParticleContainer
    : public amrex::ParticleContainerRTSoA<>
{
...
public:
    ...
    TestParticleContainer (const Vector<amrex::Geometry>            & a_geom,
        ...
        : amrex::ParticleContainerRTSoA<>(a_geom, a_dmap, a_ba, a_rr)
    {
        ...
        for(MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi)
        {
            ...
            for (IntVect iv = tile_box.smallEnd(); iv <= tile_box.bigEnd(); tile_box.next(iv))
            {
                for (int i_part=0; i_part<num_ppc;i_part++) {
                    ...
                    for (int i = 2; i < NumRuntimeIntComps(); ++i) {
        ...
        for (int lev = 0; lev <= finestLevel(); ++lev)
        {
            ...
            for(MFIter mfi = MakeMFIter(lev); mfi.isValid(); ++mfi)
            {
                ...
                AMREX_FOR_1D ( np, i,
                {
                    ...
                    {
                        AMREX_ALWAYS_ASSERT(ptd.idata(j)[i] == p.id());
                    }
                });
            }
        }
