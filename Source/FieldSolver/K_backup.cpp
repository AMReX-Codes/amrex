    ...
    Array4<Real const> const& Bz, Real dtsdx, Real dtsdy, Real dtsdy, std:string direction)
{
    ...
    const amrex::Real Mpx[3] = {
        ...
        -2*PhysConst::xi*power(Physconst::c^2) * (2*(power(Ex(j+1,k,l),2)+power(Ey(j+1,k,l),2)+power(Ez(j+1,k,l),2) -
        power(PhysConst::c,2)*(power(Bx(j+1,k,l),2)+power(By(j+1,k,l),2)+power(Bz(j+1,k,l),2)))*Bx(j+1,k,l)
       -7*(Ex(j+1,k,l)*Bx(j+1,k,l)+Ey(j+1,k,l)*By(j+1,k,l)+Ez(j+1,k,l)*Bz(j+1,k,l))*Ex(j+1,k,l)),
    
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j+1,k,l),2)+power(Ey(j+1,k,l),2)+power(Ez(j+1,k,l),2) -
        power(PhysConst::c,2)*(power(Bx(j+1,k,l),2)+power(By(j+1,k,l),2)+power(Bz(j+1,k,l),2)))*By(j+1,k,l)
        -7*(Ex(j+1,k,l)*Bx(j+1,k,l)+Ey(j+1,k,l)*By(j+1,k,l)+Ez(j+1,k,l)*Bz(j+1,k,l))*Ey(j+1,k,l)),
        
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j+1,k,l),2)+power(Ey(j+1,k,l),2)+power(Ez(j+1,k,l),2) -
        power(PhysConst::c,2)*(power(Bx(j+1,k,l),2)+power(By(j+1,k,l),2)+power(Bz(j+1,k,l),2)))*Bz(j+1,k,l)
    ...
    const amrex::Real Mpy[3] = {
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k+1,l),2)+power(Ey(j,k+1,l),2)+power(Ez(j,k+1,l),2) -
        power(PhysConst::c,2)*(power(Bx(j,k+1,l),2)+power(By(j,k+1,l),2)+power(Bz(j,k+1,l),2)))*Bx(j,k+1,l)
        -7*(Ex(j,k+1,l)*Bx(j,k+1,l)+Ey(j,k+1,l)*By(j,k+1,l)+Ez(j,k+1,l)*Bz(j,k+1,l))*Ex(j,k+1,l)),
        
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k+1,l),2)+power(Ey(j,k+1,l),2)+power(Ez(j,k+1,l),2) -
        power(PhysConst::c,2)*(power(Bx(j,k+1,l),2)+power(By(j,k+1,l),2)+power(Bz(j,k+1,l),2)))*By(j,k+1,l)
        -7*(Ex(j,k+1,l)*Bx(j,k+1,l)+Ey(j,k+1,l)*By(j,k+1,l)+Ez(j,k+1,l)*Bz(j,k+1,l))*Ey(j,k+1,l)),
    
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k+1,l),2)+power(Ey(j,k+1,l),2)+power(Ez(j,k+1,l),2) -
        power(PhysConst::c,2)*(power(Bx(j,k+1,l),2)+power(By(j,k+1,l),2)+power(Bz(j,k+1,l),2)))*Bz(j,k+1,l)
    ...
    const amrex::Real Mpz[3] = {
        ...
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k,l+1),2)+power(Ey(j,k,l+1),2)+power(Ez(j,k,l+1),2) -
        power(PhysConst::c,2)*(power(Bx(j,k,l+1),2)+power(By(j,k,l+1),2)+power(Bz(j,k,l+1),2)))*Bx(j,k,l+1)
        -7*(Ex(j,k,l+1)*Bx(j,k,l+1)+Ey(j,k,l+1)*By(j,k,l+1)+Ez(j,k,l+1)*Bz(j,k,l+1))*Ex(j,k,l+1)),
        
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k,l+1),2)+power(Ey(j,k,l+1),2)+power(Ez(j,k,l+1),2) -
        power(PhysConst::c,2)*(power(Bx(j,k,l+1),2)+power(By(j,k,l+1),2)+power(Bz(j,k,l+1),2)))*By(j,k,l+1)
        -7*(Ex(j,k,l+1)*Bx(j,k,l+1)+Ey(j,k,l+1)*By(j,k,l+1)+Ez(j,k,l+1)*Bz(j,k,l+1))*Ey(j,k,l+1)),
        
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k,l+1),2)+power(Ey(j,k,l+1),2)+power(Ez(j,k,l+1),2) -
        power(PhysConst::c,2)*(power(Bx(j,k,l+1),2)+power(By(j,k,l+1),2)+power(Bz(j,k,l+1),2)))*Bz(j,k,l+1)
    ...
    const amrex::Real beta = 4*(PhysConst::xi/PhysConst::ep0)*(power(Ex(j,k,l),2)+power(Ey(j,k,l),2)+power(Ez(j,k,l),2) -
        power(PhysConst::c,2)*(power(Bx(j,k,l),2)+power(By(j,k,l),2)+power(Bz(j,k,l),2)))+PhysConst::ep0;
    
    const amrex::Real Alpha[3] = {
    
        2*PhysConst::xi*power(PhysConst::c,2) *
    ...
    const amrex::Real Omega[3] = {
        ...
        alpha[0] + 2*PhysConst::xi*power(PhysConst::c,2) *
            (4*(Ex(j,k,l)*VxB[0]+Ey(j,k,l)*VxB[1]+Ez(j,k,l)*VxB[2])*Ex(j,k,l)
            +2*(power(Ex(j,k,l),2)+power(Ey(j,k,l),2)+power(Ez(j,k,l),2)-c^2*(power(Bx(j,k,l),2)+power(By(j,k,l),2)+power(Bz(j,k,l),2)))*VxB[0]
            +7*c^2*(VxB[0]*Bx(j,k,l)+VxB[1]*By(j,k,l)+VxB[2]*Bz(j,k,l))*Bx(j,k,l)),
        
        alpha[1] + 2*PhysConst::xi*power(PhysConst::c,2) *
            (4*(Ex(j,k,l)*VxB[0]+Ey(j,k,l)*VxB[1]+Ez(j,k,l)*VxB[2])*Ey(j,k,l)
            +2*(power(Ex(j,k,l),2)+power(Ey(j,k,l),2)+power(Ez(j,k,l),2)-c^2*(power(Bx(j,k,l),2)+power(By(j,k,l),2)+power(Bz(j,k,l),2)))*VxB[1]
            +7*c^2*(VxB[0]*Bx(j,k,l)+VxB[1]*By(j,k,l)+VxB[2]*Bz(j,k,l))*By(j,k,l)),
        
        alpha[0] + 2*PhysConst::xi*power(PhysConst::c,2) * (4*(Ex(j,k,l)*VxB[0]+Ey(j,k,l)*VxB[1] +
            Ez(j,k,l)*VxB[2])*Ez(j,k,l)+2*(power(Ex(j,k,l),2)+power(Ey(j,k,l),2)+power(Ez(j,k,l),2) - c^2*(power(Bx(j,k,l),2)+power(By(j,k,l),2)+power(Bz(j,k,l),2)))*VxB[2] + 7*c^2*(VxB[0]*Bx(j,k,l)+VxB[1]*By(j,k,l)+VxB[2]*Bz(j,k,l))*Bz(j,k,l))};
    
    // Defining matrix for the algorithm
    const amrex::Real a00 = beta + (PhysConst::xi/PhysConst::ep0) *
        ( 8*power(Ex(j,k,l),2) + 14*power(PhysConst::c,2)*power(Bx(j,k,l),2) );
    
    const amrex::Real a11 = beta +(PhysConst::xi/PhysConst::ep0) *
        ( 8*power(Ey(j,k,l),2) + 14*power(PhysConst::c,2)*power(By(j,k,l),2) );
    
    const amrex::Real a22 = beta + (PhysConst::xi/PhysConst::ep0) *
        ( 8*power(Ez(j,k,l),2) + 14*power(PhysConst::c,2)*power(Bz(j,k,l),2) );
    
    const amrex::Real a01 = (PhysConst::xi/PhysConst::ep0) *
         ( 8*Ex(j,k,l)*Ey(j,k,l) + 14*power(PhysConst::c,2)*Bx(j,k,l)*By(j,k,l) );
    
    const amrex::Real a02 = (PhysConst::xi/PhysConst::ep0) *
        ( 8*Ex(j,k,l)*Ez(j,k,l) + 14*power(PhysConst::c,2)*Bx(j,k,l)*Bz(j,k,l) );
    
    const amrex::Real a12 = (PhysConst::xi/PhysConst::ep0) *
        ( 8*Ez(j,k,l)*Ey(j,k,l) + 14*power(PhysConst::c,2)*Bz(j,k,l)*By(j,k,l) );
...
// Performing push
    ...
    Ez(j,k,l) = Ez(j,k,l) - 0.5*(invAz[0]*Omega[0]+invAz[1]*Omega[1]+invAz[2]*Omega[2])/detA;
        ...
        -2*PhysConst::xi*Physconst::c^2 * (2*(power(Ex(j+1,k,0),2)+power(Ey(j+1,k,0),2)
        -power(PhysConst::c,2)*(power(Bx(j+1,k,0),2)+power(By(j+1,k,0),2)+power(Bz(j+1,k,0),2)))*Bx(j+1,k,0)
        -7*(Ex(j+1,k,0)*Bx(j+1,k,0)+Ey(j+1,k,0)*By(j+1,k,0)+Ez(j+1,k,0)*Bz(j+1,k,0))*Ex(j+1,k,0)),
    
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j+1,k,0),2)+power(Ey(j+1,k,0),2)+power(Ez(j+1,k,0),2)
        -power(PhysConst::c,2)*(power(Bx(j+1,k,0),2)+power(By(j+1,k,0),2)+power(Bz(j+1,k,0),2)))*By(j+1,k,0)
        -7*(Ex(j+1,k,0)*Bx(j+1,k,0)+Ey(j+1,k,0)*By(j+1,k,0)+Ez(j+1,k,0)*Bz(j+1,k,0))*Ey(j+1,k,0)),
        
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j+1,k,0),2)+power(Ey(j+1,k,0),2)+power(Ez(j+1,k,0),2)
        -power(PhysConst::c,2)*(power(Bx(j+1,k,0),2)+power(By(j+1,k,0),2)+power(Bz(j+1,k,0),2)))*Bz(j+1,k,0)
    ...
    const amrex::Real Mnx[3] = {
     ...
     const amrex::Real Mpz[3] = {
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k+1,0),2)+power(Ey(j,k+1,0),2)+power(Ez(j,k+1,0),2)
        -power(PhysConst::c,2)*(power(Bx(j,k+1,0),2)+power(By(j,k+1,0),2)+power(Bz(j,k+1,0),2)))*Bx(j,k+1,0)
        -7*(Ex(j,k+1,0)*Bx(j,k+1,0)+Ey(j,k+1,0)*By(j,k+1,0)+Ez(j,k+1,0)*Bz(j,k+1,0))*Ex(j,k+1,0)),
        
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k+1,0),2)+power(Ey(j,k+1,0),2)+power(Ez(j,k+1,0),2)
        -power(PhysConst::c,2)*(power(Bx(j,k+1,0),2)+power(By(j,k+1,0),2)+power(Bz(j,k+1,0),2)))*By(j,k+1,0)
        -7*(Ex(j,k+1,0)*Bx(j,k+1,0)+Ey(j,k+1,0)*By(j,k+1,0)+Ez(j,k+1,0)*Bz(j,k+1,0))*Ey(j,k+1,0)),
    
        -2*PhysConst::xi*power(PhysConst::c,2) * (2*(power(Ex(j,k+1,0),2)+power(Ey(j,k+1,0),2)+power(Ez(j,k+1,0),2)
        -power(PhysConst::c,2)*(power(Bx(j,k+1,0),2)+power(By(j,k+1,0),2)+power(Bz(j,k+1,0),2)))*Bz(j,k+1,0)
    ...
    const amrex::Real beta =
        4*(PhysConst::xi/PhysConst::ep0)*(power(Ex(j,k,0),2)+power(Ey(j,k,0),2)+power(Ez(j,k,0),2)-
        power(PhysConst::c,2)*(power(Bx(j,k,0),2)+power(By(j,k,0),2)+power(Bz(j,k,0),2)))+PhysConst::ep0;
    ...
    const amrex::Real Omega[3] = {
        ...
        alpha[0] + 2*PhysConst::xi*power(PhysConst::c,2) *
            ...
            + 2*(power(Ex(j,k,0),2)+power(Ey(j,k,0),2)+power(Ez(j,k,0),2)-c^2*(power(Bx(j,k,0),2)+power(By(j,k,0),2)+power(Bz(j,k,0),2)))*VxB[0]
            + 7*c^2*(VxB[0]*Bx(j,k,0)+VxB[1]*By(j,k,0)+VxB[2]*Bz(j,k,0))*Bx(j,k,0)),
        
        alpha[1] + 2*PhysConst::xi*power(PhysConst::c,2) *
            (4*(Ex(j,k,0)*VxB[0]+Ey(j,k,0)*VxB[1]+Ez(j,k,0)*VxB[2])*Ey(j,k,0)
            +2*(power(Ex(j,k,0),2)+power(Ey(j,k,0),2)+power(Ez(j,k,0),2)-c^2*(power(Bx(j,k,0),2)+power(By(j,k,0),2)+power(Bz(j,k,0),2)))*VxB[1]
            +7*c^2*(VxB[0]*Bx(j,k,0)+VxB[1]*By(j,k,0)+VxB[2]*Bz(j,k,0))*By(j,k,0)),
        
        alpha[0] + 2*PhysConst::xi*power(PhysConst::c,2)*
            (4*(Ex(j,k,0)*VxB[0]+Ey(j,k,0)*VxB[1]+Ez(j,k,0)*VxB[2])*Ez(j,k,0)
            +2*(power(Ex(j,k,0),2)+power(Ey(j,k,0),2)+power(Ez(j,k,0),2)-c^2*(power(Bx(j,k,0),2)+power(By(j,k,0),2)+power(Bz(j,k,0),2)))*VxB[2]
            +7*c^2*(VxB[0]*Bx(j,k,0)+VxB[1]*By(j,k,0)+VxB[2]*Bz(j,k,0))*Bz(j,k,0))};
    
    const amrex::Real a00 = beta + (PhysConst::xi/PhysConst::ep0) * ( 8*power(Ex(j,k,0),2) + 14*power(PhysConst::c,2)*power(Bx(j,k,0),2) );
    const amrex::Real a11 = beta + (PhysConst::xi/PhysConst::ep0) * ( 8*power(Ey(j,k,0),2) + 14*power(PhysConst::c,2)*power(By(j,k,0),2) );
    const amrex::Real a22 = beta + (PhysConst::xi/PhysConst::ep0) * ( 8*power(Ez(j,k,0),2) + 14*power(PhysConst::c,2)*power(Bz(j,k,0),2) );
    
    const amrex::Real a01 = (PhysConst::xi/PhysConst::ep0) * ( 8*Ex(j,k,0)*Ey(j,k,0) + 14*power(PhysConst::c,2)*Bx(j,k,0)*By(j,k,0) );
    const amrex::Real a02 = (PhysConst::xi/PhysConst::ep0) * ( 8*Ex(j,k,0)*Ez(j,k,0) + 14*power(PhysConst::c,2)*Bx(j,k,0)*Bz(j,k,0) );
    const amrex::Real a12 = (PhysConst::xi/PhysConst::ep0) * ( 8*Ez(j,k,0)*Ey(j,k,0) + 14*power(PhysConst::c,2)*Bz(j,k,0)*By(j,k,0) );
