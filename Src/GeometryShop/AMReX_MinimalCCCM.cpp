                                          ...
                                          const int  & a_degreePmax)
{
  for (int iDegree = 0; iDegree <= a_degreePmax; ++iDegree)
  {
    ...
    IndexTM<int,1>degree;
    degree[0] = iDegree;
    m_cutCellMoments.m_moments[degree] = std::pow(a_hiPt,iDegree + 1) - std::pow(a_loPt,iDegree +1);
                                                 ...
                                                 const int  & a_degreePmax)
{
  for (int iDegree = 0; iDegree <= a_degreePmax; ++iDegree)
  {
    ...
    for (int j = 1; j <= iDegree + 1; j++)
    {
      ...
      if (a_loSign >= ON)
      {
        m_cutCellMoments.m_moments[degree] += factor * std::pow(a_loPt,iDegree + 1 - j) * std::pow(epsilon,j);
      }
      else if (a_hiSign >= ON)
      {
        m_cutCellMoments.m_moments[degree] -= factor * std::pow(a_hiPt,iDegree + 1 - j) * std::pow(epsilon,j) * std::pow(-1.0,j);
      }
    }

    //Real dxFactor = std::pow(m_cutCellMoments.m_IFData.m_globalCoord.m_dx[0],iDegree + 1);
    //m_cutCellMoments.m_moments[degree] *= dxFactor;
    m_cutCellMoments.m_moments[degree] /= iDegree + 1;
  }
}
