  ...
  if((s_knownFunc == 1) && (a_deriv.sum() <= 4))
    {
      ...
      if(Abs(rightAns - a_myAnswer) > tol)
        {
      ...
      const RealVect& a_point) const
{
  ...
  if(order == 0)
    ...
    {
      MayDay::Error("derivs higher than 4 not programmed in this class");
    }
  return retval;
}
////
      ...
      const RealVect& a_point) const
{
  if(SpaceDim != 2)
    {
      MayDay::Error("offset sphere now currently only handles 2d");
    }
          ...
          const RealVect& a_point) const
{
  ...
  if(!found)
    {
      MayDay::Error("logic error offsph1.0");
    }
               ...
               const RealVect& a_point)  const
{
  ...
  if(a_deriv.max() == 2)
    ...
    {
      MayDay::Error("logic error offsph2");
    }
  return retval;
}
////
              ...
              const RealVect& a_point) const
{
  ...
  if(a_deriv.max() == 3)
    ...
    {
      MayDay::Error("logic error offsph3 (maybe running in 3d?)");
    }
  return retval;
}
////
               ...
               const RealVect& a_point) const
{
  ...
  if(a_deriv.max() == 4)
    ...
    {
      MayDay::Error("logic error offsph4");
    }
  return retval;
}
