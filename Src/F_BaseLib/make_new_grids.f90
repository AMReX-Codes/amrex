module make_new_grids_module
  ...
  contains
    ...
    subroutine make_boxes(mf,ba_new,dx_crse,buf_wid,lev,aux_tag_mf,ref_ratio)
      ...
      call lmultifab_destroy(tagboxes)
    ...
    subroutine buffer(lev,la_np1,la_nm1,ba_new,ref_ratio,buff)
      ...
      call boxarray_build_copy(boxes,get_boxarray(la_np1))
        ...
        do while ( (nl .ge. 2) )
            ...
            if (.not. empty(mba%bas(nl+1))) then
              ...
              if (.not. ml_boxarray_properly_nested(mba, ng_buffer, pmask, nl+1, nl+1)) then
                ...
                call layout_build_ba(latmp,mba%bas(nl),mba%pd(nl))
                call layout_boxarray_diff(ba_old_comp,mba%pd(nl),latmp)
                call layout_destroy(latmp)

                call layout_build_ba(la_old_comp,ba_old_comp,mba%pd(nl),mapping = LA_LOCAL)
                ...
                call boxarray_build_l(ba_newest,bl)
                ...
                call destroy(bl)
                call boxarray_destroy(ba_new)
                call boxarray_destroy(ba_old_comp)
                call layout_destroy(la_old_comp)

                ! Replace mba%bas(nl) by ba_newest
                call boxarray_destroy(mba%bas(nl))
                call boxarray_build_copy(mba%bas(nl),ba_newest)
                call boxarray_destroy(ba_newest)

                ! Destroy the old layout and build a new one.
                call layout_destroy(la_array(nl))
                ...
                call setval(tagboxes, .false.)
                call boxarray_build_copy(ba_crse_fine,mba%bas(nl))
                ...
                call boxarray_destroy(mba%bas(nl))
                call boxarray_build_copy(mba%bas(nl),ba_new)
