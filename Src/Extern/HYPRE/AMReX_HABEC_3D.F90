module amrex_habec_module
  ...
  use amrex_petsc_fort_module, only : int=>petsc_int
#else
  use amrex_hypre_fort_module, only : int=>hypre_int
#endif
...
contains
  ...
  subroutine amrex_hpijmatrix (lo, hi, nrows, ncols, rows, cols, mat, &
    ...
    integer(int), intent(in) :: nrows, cell_id_begin;
    integer(int), dimension(0:nrows-1), intent(out) :: ncols, rows
    integer(int), dimension(0:nrows*7-1), intent(out) :: cols
    real(rt)    , dimension(0:nrows*7-1), intent(out) :: mat
    integer, dimension(3), intent(in) :: lo, hi, clo, chi, dlo, dhi, alo, ahi,&
         xlo, xhi, ylo, yhi, zlo, zhi
    integer(int), intent(in) :: cell_id(clo(1):chi(1),clo(2):chi(2),clo(3):chi(3))
    real(rt)    , intent(inout) :: diag(dlo(1):dhi(1),dlo(2):dhi(2),dlo(3):dhi(3))
    real(rt)    , intent(in) :: a      (alo(1):ahi(1),alo(2):ahi(2),alo(3):ahi(3))
  ...
  subroutine amrex_hpeb_fill_cellid (lo, hi, nrows, cell_id, clo, chi, flag, flo, fhi) &
    ...
    integer(int), intent(  out) :: nrows
    integer(int), intent(inout) :: cell_id(clo(1):chi(1),clo(2):chi(2),clo(3):chi(3))
  ...
  subroutine amrex_hpeb_ijmatrix (lo, hi, nrows, ncols, rows, cols, mat, &
    ...
    integer(int), intent(in) :: nrows, cell_id_begin;
    integer(int), dimension(0:nrows-1), intent(out) :: ncols, rows
    integer(int), dimension(0:nrows*27-1), intent(out) :: cols
