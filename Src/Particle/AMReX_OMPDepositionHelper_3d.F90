subroutine amrex_atomic_accumulate_fab(local_fab, tile_lo, tile_hi, &
  ...
  integer              :: lo(3)
  integer              :: hi(3)
  real(amrex_real)     :: global_fab(lo(1):hi(1), lo(2):hi(2), lo(3):hi(3))
  integer              :: tile_lo(3)
  integer              :: tile_hi(3)
  real(amrex_real)     :: local_fab(tile_lo(1):tile_hi(1), tile_lo(2):tile_hi(2), tile_lo(3):tile_hi(3))
  ...
  do k = tile_lo(3), tile_hi(3)
     do j = tile_lo(2), tile_hi(2)
        do i = tile_lo(1), tile_hi(1)
           !$omp atomic
           global_fab(i,j,k) = global_fab(i,j,k) + local_fab(i,j,k)
           !$omp end atomic
        end do
     end do
  end do
