...
TracerParticleContainer::AdvectWithUmac (MultiFab* umac, int lev, Real dt)
{
	...
	for (auto& kv : pmap) {
	  ...
	  FArrayBox* fab[BL_SPACEDIM] = { BL_D_DECL(&((*umac_pointer[0])[grid]),
						 &((*umac_pointer[1])[grid]),
						 &((*umac_pointer[2])[grid])) };
...
#pragma omp parallel for
            ...
            for (int i = 0; i < n; i++)
            {
                ...
                const Real len[BL_SPACEDIM] = { BL_D_DECL((p.m_rdata.pos[0]-plo[0])/dx[0] + Real(0.5),
                                                       (p.m_rdata.pos[1]-plo[1])/dx[1] + Real(0.5),
                                                       (p.m_rdata.pos[2]-plo[2])/dx[2] + Real(0.5)) };

                const IntVect cell(BL_D_DECL(floor(len[0]), floor(len[1]), floor(len[2])));

                const Real frac[BL_SPACEDIM] = { BL_D_DECL(len[0]-cell[0], len[1]-cell[1], len[2]-cell[2]) };
                ...
                for (int d = 0; d < BL_SPACEDIM; d++)
                {
                    ...
                    Real efrac[BL_SPACEDIM] = { BL_D_DECL(frac[0], frac[1], frac[2]) };
...
TracerParticleContainer::AdvectWithUcc (const MultiFab& Ucc, int lev, Real dt)
{
    ...
    int idx[BL_SPACEDIM] = {BL_D_DECL(0,1,2)};
