...
module amrex_plotfile_module
  ...
  public :: amrex_write_hdf5plotfile
#endif
...
#ifdef AMREX_USE_HDF5
     subroutine amrex_fi_write_hdf5plotfile (name, nlevs, mf, varname, geom, t, steps, rr) bind(c)
       import
       implicit none
       character(kind=c_char), intent(in) :: name(*)
       integer(c_int), value :: nlevs
       ...
       real(amrex_real), value :: t
       integer(c_int), intent(in) :: steps(*), rr(*)
     end subroutine amrex_fi_write_hdf5plotfile
#endif
  end interface
...
#ifdef AMREX_USE_HDF5
  subroutine amrex_write_hdf5plotfile (name, nlevs, mf, varname, geom, t, steps, rr)
    character(len=*), intent(in) :: name
    integer, intent(in) :: nlevs, steps(*), rr(*)
    type(amrex_multifab), intent(in) :: mf(*)
    type(amrex_string), intent(in), target :: varname(*)
    ...
    call amrex_fi_write_hdf5plotfile(amrex_string_f_to_c(name), nlevs, mf_c, varname_c, geom_c, &
         t, steps, rr)
  end subroutine amrex_write_hdf5plotfile
#endif
