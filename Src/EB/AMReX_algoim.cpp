...
//            AMREX_HOST_DEVICE_FOR_3D ( bx, i, j, k,
            ...
            for (int i = lo.x; i <= hi.x; ++i)
            {
                ...
                if (ebflag.isRegular()) {
                    ...
                    intg(i,j,k,i_S_x    ) = q([](Real x, Real y, Real z)
                                              { return x; });
                    intg(i,j,k,i_S_y    ) = q([](Real x, Real y, Real z)
                                              { return y; });
                    intg(i,j,k,i_S_z    ) = q([](Real x, Real y, Real z)
                                              { return z; });
                    intg(i,j,k,i_S_x2   ) = q([](Real x, Real y, Real z)
                                              { return x*x; });
                    intg(i,j,k,i_S_y2   ) = q([](Real x, Real y, Real z)
                                              { return y*y; });
                    intg(i,j,k,i_S_z2   ) = q([](Real x, Real y, Real z)
                                              { return z*z; });
                    intg(i,j,k,i_S_x_y  ) = q([](Real x, Real y, Real z)
                                              { return x*y; });
                    intg(i,j,k,i_S_x_z  ) = q([](Real x, Real y, Real z)
                                              { return x*z; });
                    intg(i,j,k,i_S_y_z  ) = q([](Real x, Real y, Real z)
                                              { return y*z; });
                    intg(i,j,k,i_S_x2_y ) = q([](Real x, Real y, Real z)
                                              { return x*x*y; });
                    intg(i,j,k,i_S_x2_z ) = q([](Real x, Real y, Real z)
                                              { return x*x*z; });
                    intg(i,j,k,i_S_x_y2 ) = q([](Real x, Real y, Real z)
                                              { return x*y*y; });
                    intg(i,j,k,i_S_y2_z ) = q([](Real x, Real y, Real z)
                                              { return y*y*z; });
                    intg(i,j,k,i_S_x_z2 ) = q([](Real x, Real y, Real z)
                                              { return x*z*z; });
                    intg(i,j,k,i_S_y_z2 ) = q([](Real x, Real y, Real z)
                                              { return y*z*z; });
                    intg(i,j,k,i_S_x2_y2) = q([](Real x, Real y, Real z)
                                              { return x*x*y*y; });
                    intg(i,j,k,i_S_x2_z2) = q([](Real x, Real y, Real z)
                                              { return x*x*z*z; });
                    intg(i,j,k,i_S_y2_z2) = q([](Real x, Real y, Real z)
                                              { return y*y*z*z; });
                }
            }
        }
