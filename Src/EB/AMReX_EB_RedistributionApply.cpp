                        ...
                        Array4<Real const> const& srd_update_scale)
{
    ...
    if (redistribution_type == "FluxRedist")
    {
        ...
        amrex_flux_redistribute (bx, dUdt_out, dUdt_in, wt, vfrac, flag,
                                 as_crse, rr_drho_crse, rr_flag_crse,
                                 as_fine, dm_as_fine, levmsk, lev_geom,
                                 use_wts_in_divnc, level_mask_not_covered, icomp, ncomp, dt);

    } else if (redistribution_type == "StateRedist") {

        // Currently we don't have the re-redistribution version of StateRedistribution
        // This is a work in progress so for now we use only the non-re-re version
        ApplyRedistribution (bx, ncomp, dUdt_out, dUdt_in, U_in, wt, flag,
