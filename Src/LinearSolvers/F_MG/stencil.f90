...
! end function f_bc_bit
  ...
  subroutine extrap_1d(ph, mm, xa, xb, dh, max_order, cross)
    ...
    xx(0,1) = -xa(1)/dh(1)
    xx(0,2) = -xb(1)/dh(1)
    call poly_interp_coeff(cc(0:norder(1)-1,1), -HALF, xx(0:norder(1)-1,1))
    call poly_interp_coeff(cc(0:norder(1)-1,2), -HALF, xx(0:norder(1)-1,2))
    if ( bc_dirichlet(mm( 1), 1, -1) ) &
         ph(0   ) = sum(ph(1:norder(1)-1         )*cc(1:norder(1)-1,1))
    if ( bc_dirichlet(mm(nn(1)), 1, +1) ) &
         ph(nn(1)+1) = sum(ph(nn(1):nn(1)-(norder(1)-2):-1)*cc(1:norder(1)-1,2))
  ...
  subroutine extrap_2d(ph, mm, xa, xb, dh, max_order, cross)
    ...
    xx(0,1) = -xa(2)/dh(2)
    xx(0,2) = -xb(2)/dh(2)
    call poly_interp_coeff(cc(0:norder(2)-1,1), -HALF, xx(0:norder(2)-1,1))
  ...
  subroutine extrap_3d(ph, mm, xa, xb, dh, max_order, cross)
    ...
    xx(0,1) = -xa(3)/dh(3)
    xx(0,2) = -xb(3)/dh(3)
    call poly_interp_coeff(cc(0:norder(3)-1,1), -HALF, xx(0:norder(3)-1,1))
    call poly_interp_coeff(cc(0:norder(3)-1,2), -HALF, xx(0:norder(3)-1,2))
    do j = 1, nn(2)
       do i = 1, nn(1)
...
module coeffs_module
  ...
  private crse_coeffs_1d
  private crse_coeffs_2d
  private crse_coeffs_3d

contains

  subroutine coarsen_coeffs(cf, cc)
    type(multifab), intent(in)    :: cc
    type(multifab), intent(inout) :: cf
    ...
    do i = 1, cf%nboxes
       ...
       select case (cf%dim)
       case (1)
          call crse_coeffs_1d(ccp(:,1,1,:), cfp(:,1,1,:))
       case (2)
          call crse_coeffs_2d(ccp(:,:,1,:), cfp(:,:,1,:))
       case (3)
          call crse_coeffs_3d(ccp(:,:,:,:), cfp(:,:,:,:))
       end select
    end do
  end subroutine coarsen_coeffs

  subroutine crse_coeffs_1d(cc, cf)
    real(kind=dp_t), intent(inout) :: cc(-1:,0:)
    real(kind=dp_t), intent(in)    :: cf(-1:,0:)
    integer i,i2,nx
  ...
  subroutine crse_coeffs_2d(cc, cf)
    real(kind=dp_t), intent(inout) :: cc(-1:,-1:,0:)
    real(kind=dp_t), intent(in)    :: cf(-1:,-1:,0:)
    integer i,i2,j,j2,nx,ny
  ...
  subroutine crse_coeffs_3d(cc, cf)
    real(kind=dp_t), intent(inout) :: cc(-1:,-1:,-1:,0:)
    real(kind=dp_t), intent(in)    :: cf(-1:,-1:,-1:,0:)
    integer i, i2, j, j2, k, k2, nx, ny, nz
