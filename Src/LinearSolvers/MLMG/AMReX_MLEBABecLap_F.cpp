...
MLEBABecLap::Fapply (int amrlev, int mglev, MultiFab& out, const MultiFab& in) const
{
    ...
    if (Gpu::inLaunchRegion() && in.isFusingCandidate()) {
        ...
        auto const& phiebma = (is_eb_dirichlet && is_eb_inhomog)
            ? m_eb_phi[amrlev]->const_arrays() : foo;
...
MLEBABecLap::Fsmooth (int amrlev, int mglev, MultiFab& sol, const MultiFab& rhs, int redblack) const
{
    ...
    if (Gpu::inLaunchRegion() && sol.isFusingCandidate()) {
        ...
        const auto& m1 = mm1.const_arrays();
#if (AMREX_SPACEDIM > 1)
        const auto& m2 = mm2.const_arrays();
        const auto& m3 = mm3.const_arrays();
#if (AMREX_SPACEDIM > 2)
        const auto& m4 = mm4.const_arrays();
        const auto& m5 = mm5.const_arrays();
#endif
#endif
