...
contains
  ...
  subroutine amrex_mlpoisson_adotx (lo, hi, y, ylo, yhi, x, xlo, xhi, dxinv) &
    ...
    do    j = lo(2), hi(2)
       do i = lo(1), hi(1)
          y(i,j) = dhx * (x(i-1,j) - 2.d0*x(i,j) + x(i+1,j)) &
               +   dhy * (x(i,j-1) - 2.d0*x(i,j) + x(i,j+1))
       end do
    end do
  ...
  subroutine amrex_mlpoisson_flux (lo, hi, fx, fxlo, fxhi, fy, fylo, fyhi, &
       sol, slo, shi, dxinv, face_only) bind(c, name='amrex_mlpoisson_flux')
    ...
    if (face_only .eq. 1) then
       do    j = lo(2), hi(2)
          do i = lo(1), hi(1)+1, hi(1)+1-lo(1)
             fx(i,j) = dhx * (sol(i,j) - sol(i-1,j))
          end do
       end do
       
       do    j = lo(2), hi(2)+1, hi(2)+1-lo(2)
          do i = lo(1), hi(1)
             fy(i,j) = dhy * (sol(i,j) - sol(i,j-1))
          end do
       end do
  ...
  subroutine amrex_mlpoisson_gsrb (lo, hi, phi, hlo, hhi, rhs, rlo, rhi, &
    ...
    do j = lo(2), hi(2)
       ...
       do i = lo(1) + ioff, hi(1), 2
          ...
          g_m_d = gamma + dhx*(cf0+cf2) + dhy*(cf1+cf3)

          res = rhs(i,j) - gamma*phi(i,j) &
               - dhx*(phi(i-1,j) + phi(i+1,j))  &
               - dhy*(phi(i,j-1) + phi(i,j+1))
