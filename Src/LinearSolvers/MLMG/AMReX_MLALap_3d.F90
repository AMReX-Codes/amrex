...
contains
  ...
  subroutine amrex_mlalap_adotx (lo, hi, y, ylo, yhi, x, xlo, xhi, a, alo, ahi, &
       dxinv, alpha, beta) bind(c,name='amrex_mlalap_adotx')
    integer, dimension(3), intent(in) :: lo, hi, ylo, yhi, xlo, xhi, alo, ahi
       ...
       do    j = lo(2), hi(2)
          do i = lo(1), hi(1)
             y(i,j,k) = alpha*a(i,j,k)*x(i,j,k) &
                  - dhx * (x(i-1,j,k) - 2.d0*x(i,j,k) + x(i+1,j,k)) &
  ...
  subroutine amrex_mlalap_flux (lo, hi, fx, fxlo, fxhi, fy, fylo, fyhi, &
       fz, fzlo, fzhi, sol, slo, shi, dxinv, beta, face_only) bind(c, name='amrex_mlalap_flux')
  ...
  subroutine amrex_mlalap_gsrb (lo, hi, phi, hlo, hhi, rhs, rlo, rhi, &
       ...
       a, alo, ahi, blo, bhi, dxinv, alpha, beta, redblack) bind(c,name='amrex_mlalap_gsrb')
    integer, dimension(3), intent(in) :: lo, hi, hlo, hhi, rlo, rhi, &
         f0lo,f0hi,f1lo,f1hi,f2lo,f2hi,f3lo,f3hi,f4lo,f4hi,f5lo,f5hi, &
         m0lo,m0hi,m1lo,m1hi,m2lo,m2hi,m3lo,m3hi,m4lo,m4hi,m5lo,m5hi,alo,ahi,blo, bhi
    integer, intent(in), value :: redblack
    real(amrex_real), intent(in) :: dxinv(3), alpha, beta
    ...
    real(amrex_real) :: gamma, g_m_d, res, dhfac, rho
    ...
    do k = lo(3), hi(3)
       do j = lo(2), hi(2)
          ...
          do i = lo(1) + ioff, hi(1), 2
             ...
             gamma = alpha*a(i,j,k) + dhfac

             g_m_d = gamma - dhx*(cf0+cf3) - dhy*(cf1+cf4) - dhz*(cf2+cf5)

             rho =  dhx*(phi(i-1,j,k) + phi(i+1,j,k))  &
...
end module amrex_mlalap_3d_module
