...
#pragma omp parallel if (Gpu::notInLaunchRegion())
    ...
    for (MFIter mfi(out, TilingIfNotGPU()); mfi.isValid(); ++mfi)
    {
        ...
        if (m_overset_mask[amrlev][mglev]) {
            ...
            AMREX_LAUNCH_HOST_DEVICE_FUSABLE_LAMBDA ( bx, tbx,
            {
...
#pragma omp parallel if (Gpu::notInLaunchRegion())
	...
	const Box& tbx = mfi.tilebox();
        ...
        if (m_overset_mask[amrlev][mglev]) {
            const auto& osm = m_overset_mask[amrlev][mglev]->array(mfi);
            AMREX_LAUNCH_HOST_DEVICE_FUSABLE_LAMBDA ( tbx, thread_box,
            {
                abec_gsrb_os(thread_box, solnfab, rhsfab, alpha, afab,
                             AMREX_D_DECL(dhx, dhy, dhz),
                             AMREX_D_DECL(bxfab, byfab, bzfab),
                        ...
                        FArrayBox const& sol, int face_only, int ncomp)
{
    ...
    if (face_only)
    {
        ...
        AMREX_LAUNCH_HOST_DEVICE_FUSABLE_LAMBDA ( blo, tbox,
        {
    ...
    else
    {
        ...
        AMREX_LAUNCH_HOST_DEVICE_FUSABLE_LAMBDA ( bflux, tbox,
        {
