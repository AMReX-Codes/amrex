    ...
    beta(beta_def)
{
    ...
    BL_D_TERM(_hh[0]=_h;, _hh[1]=_h;, _hh[2]=_h;);
...
#pragma omp parallel
	...
	BL_D_TERM(const Mask& mw = mmw[solnLmfi];,
               const Mask& ms = mms[solnLmfi];,
               const Mask& mb = mmb[solnLmfi];);

	BL_D_TERM(const Mask& me = mme[solnLmfi];,
...
#pragma omp parallel
	...
	BL_D_TERM(const Mask& mw = mmw[xmfi];,
               const Mask& ms = mms[xmfi];,
               const Mask& mb = mmb[xmfi];);

	BL_D_TERM(const Mask& me = mme[xmfi];,
        ...
        BL_D_TERM(const FArrayBox& bxfab = bX[xmfi];,
               const FArrayBox& byfab = bY[xmfi];,
               const FArrayBox& bzfab = bZ[xmfi];);

        BL_D_TERM(FArrayBox& xfluxfab = xflux[xmfi];,
