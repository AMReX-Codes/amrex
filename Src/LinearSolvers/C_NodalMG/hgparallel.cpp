...
#if 0
void
task_list::execute_1 (const char* msg)
{
  BL_PROFILE(BL_PROFILE_THIS_NAME() + "::execute_1()");
  ...
  for ( std::list<task::task_proxy>::iterator tli = tasks.begin(); tli != tasks.end(); ++tli )
    {
      int with = -1;
      if ((*tli)->need_to_communicate(with))
        {
  ...
  std::vector<MPI_Request> tpi;
  std::vector<task::task_proxy*> tpp;

  while (!tasks.empty())
    {
      for ( std::list<task::task_proxy>::iterator tli = tasks.begin(); tli != tasks.end(); )
        {
	  task::task_proxy t = *tli;
	  ...
	  if ( t->depend_ready() )
	      ...
	      if ( !t->is_started() )
                {
		  t->startup();
		  if ( t->get_request() != MPI_REQUEST_NULL )
		    {
		      task::task_proxy* tp = new task::task_proxy(t);
		      bool found = false;
		      tasks.erase(tli);
		      for ( int i = 0; i < tpi.size(); ++i )
			{
			  if ( tpi[i] == MPI_REQUEST_NULL )
			    {
			      tpi[i] = t->get_request();
			      tpp[i] = tp;
			      found = true;
      ...
      {
	int outcount;
	std::vector<int> oai(tpi.size());
	std::vector<MPI_Status> oas(tpi.size());
#ifdef BL_USE_MPI
	MPI_Waitsome(tpi.size(), &tpi[0], &outcount, &oai[0], &oas[0]);
#endif
	for ( int i = 0; i < outcount; ++i )
	  {
	    task::task_proxy* tp = tpp[oai[i]];
	    (*tp)->ready();
	    tp->set_finished();
	    delete tp;
	  }
