...
int
VisMF::realDescriptorNumBytes () const
{
    if (m_hdr.m_ba.empty()) {
        return 0;
    }

    if (VisMF::NoFabHeader(m_hdr)) {
        return m_hdr.m_writtenRD.numBytes();
    }

    std::string FullName = VisMF::DirName(m_fafabname);
    FullName += m_hdr.m_fod[0].m_name;
    ...
    ifs.seekg(m_hdr.m_fod[0].m_head, std::ios::beg);

    char c;
    ifs >> c;
    if (c != 'F') { amrex::Error("VisMF::realDescriptorNumBytes(): expected F"); }
    ifs >> c;
    if (c != 'A') { amrex::Error("VisMF::realDescriptorNumBytes(): expected A"); }
    ifs >> c;
    if (c != 'B') { amrex::Error("VisMF::realDescriptorNumBytes(): expected B"); }
    ...
    if (c == ':') {
        ...
        case FABio::FAB_IEEE:
            if (wrd_in == FABio::FAB_FLOAT) {
                return 4;
            } else if (wrd_in == FABio::FAB_DOUBLE) {
                return 8;
            }
            break;
        ...
        amrex::Error("VisMF::realDescriptorNumBytes(): unrecognized FABio header");
    } else {
        ifs.putback(c);
        RealDescriptor rd;
        ifs >> rd;
