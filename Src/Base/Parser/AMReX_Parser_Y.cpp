...
parser_ast_optimize (struct parser_node* node)
{
    ...
    case PARSER_MUL:
        ...
        { // 3 * (4*x)
            ...
            node->r = node->r->r;
            parser_ast_optimize(node); // The number might be 1.
        }
        else if (node->l->type == PARSER_NUMBER &&
                 node->r->type == PARSER_DIV &&
                 node->r->l->type == PARSER_NUMBER)
    ...
    case PARSER_F3:
        ...
        break;
    case PARSER_USRF1:
        parser_ast_optimize(((struct parser_usrf1*)node)->l);
        break;
    case PARSER_USRF2:
        parser_ast_optimize(((struct parser_usrf2*)node)->l);
        parser_ast_optimize(((struct parser_usrf2*)node)->r);
        break;
    case PARSER_USRFN:
    {
        parser_ast_optimize(((struct parser_usrfn*)node)->n1);
        short argc = ((struct parser_usrfn*)node)->argc;
        for (short iarg = 0; iarg < argc-1; ++iarg) {
            parser_ast_optimize(((struct parser_usrfn*)node)->others[iarg]);
        }
        break;
    }
    case PARSER_ASSIGN:
