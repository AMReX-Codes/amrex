                                         ...
                                         const Real* time, const int* bc) const
{
	...
	m_func3D(data,ARLIM_3D(lo),ARLIM_3D(hi),ARLIM_3D(dom_lo),ARLIM_3D(dom_hi),ZFILL(dx),ZFILL(grd_lo),time,BCREC_3D(bc));
    } else {
#ifdef _OPENMP
                                         ...
                                         const Real* time, const int* bc, int ng) const
{
    ...
    if (thread_safe) {
	  ...
	  m_gfunc3D(data,ARLIM_3D(lo),ARLIM_3D(hi),ARLIM_3D(dom_lo),ARLIM_3D(dom_hi),ZFILL(dx),ZFILL(grd_lo),time,BCREC_3D(bc));
    } else {
#ifdef _OPENMP
#pragma omp critical (bndryfunc)
#endif
