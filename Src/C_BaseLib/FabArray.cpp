...
static std::map< upcxx::rank_t, PgasSendInfo > pgas_send_info_map;
          ...
          int SeqNum)
{
  ...
  if (it != pgas_send_info_map.end()) {
    ...
    PgasSendInfo send_info = PgasSendInfo(src_ptr, dst_ptr, nbytes, SeqNum);
    pgas_send_info_map.insert(std::pair<upcxx::rank_t, PgasSendInfo>(dst_ptr.where(), send_info));
  }
}
#endif /* BL_USE_UPCXX */
