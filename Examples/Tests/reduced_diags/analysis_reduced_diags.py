...
# Tolerance: 1.0e-10 for particle energy, 1.0e-3 for field energy.
...
for i in range(w.size):
    EPyt = EPyt + (math.sqrt((px[i]**2+py[i]**2+pz[i]**2)*c**2+me**2*c**4)-me*c**2)*w[i]
# proton
...
for i in range(w.size):
    EPyt = EPyt + (math.sqrt((px[i]**2+py[i]**2+pz[i]**2)*c**2+mp**2*c**4)-mp*c**2)*w[i]
...
dV = N[0]*N[1]*N[2]
EFyt = 0.5*Es*ep0*dV + 0.5*Bs/mu0*dV
...
with open('./diags/reducedfiles/EF.txt') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 2:
            EF = numpy.array(row[2])
            EF = EF.astype(numpy.float)
        line_count += 1
...
with open('./diags/reducedfiles/EP.txt') as csv_file:
    ...
    for row in csv_reader:
        if line_count == 2:
            EP = numpy.array(row[2])
            EP = EP.astype(numpy.float)
        line_count += 1
