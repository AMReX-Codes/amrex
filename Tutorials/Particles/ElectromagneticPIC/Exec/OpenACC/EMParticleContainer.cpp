                       ...
                       MultiFab& jx, MultiFab& jy, MultiFab& jz, Real dt)
{
	...
	gather_magnetic_field_acc(np, structs.dataPtr(),
	...
	gather_electric_field_acc(np, structs.dataPtr(),
        ...
        push_momentum_boris_acc(np, uxp.dataPtr(), uyp.dataPtr(), uzp.dataPtr(), ginv.dataPtr(),
                                  Exp.dataPtr(), Eyp.dataPtr(), Ezp.dataPtr(),
                                  Bxp.dataPtr(), Byp.dataPtr(), Bzp.dataPtr(),
                                  m_charge, m_mass, dt);

        push_position_boris_acc(np, structs.dataPtr(),
                                  uxp.dataPtr(), uyp.dataPtr(), uzp.dataPtr(), ginv.dataPtr(), dt);

        deposit_current_acc(BL_TO_FORTRAN_3D(jx[pti]),
...
void EMParticleContainer::PushParticlePositions(Real dt)
{
    ...
    for (EMParIter pti(*this, lev); pti.isValid(); ++pti)
    {
        ...
        set_gamma_acc(np, uxp.dataPtr(), uyp.dataPtr(), uzp.dataPtr(), ginv.dataPtr());
