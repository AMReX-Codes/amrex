...
void splitBox(int split, int dir, const Box& box, Box& left, Box& right) {
...
void build_kd_tree_recursive(KDNode* node, const FArrayBox& cost) {
    if (node->num_procs_left == 1) return;
    
    partitionNode(node, cost);

    build_kd_tree_recursive(node->left,  cost);
    build_kd_tree_recursive(node->right, cost);
}

KDNode* build_kd_tree(const Box& domain, const FArrayBox& cost, int num_procs) {
    Real total_cost = cost.sum(0);
    KDNode* root = new KDNode(domain, total_cost, num_procs);
    build_kd_tree_recursive(root, cost);
    return root;
}

void walk_kd_tree(KDNode* node, BoxList& bl, Array<Real>& costs) {
    ...
    walk_kd_tree(node->left, bl, costs);
    walk_kd_tree(node->right, bl, costs);
}
...
int main(int argc, char* argv[])
{
    ...
    walk_kd_tree(root, new_bl, box_costs);
