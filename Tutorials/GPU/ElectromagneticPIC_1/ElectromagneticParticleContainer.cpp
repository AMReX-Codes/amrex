                        ...
                        amrex::Real charge, amrex::Real mass, amrex::Real dt)
{
    ...
    getThreadIndex(index, threadSize, pData.size());
...
{   
    ...
    for (MFIter mfi(*m_mask_ptr, false); mfi.isValid(); ++mfi)
    {
        ...
        AMREX_CUDA_LAUNCH_GLOBAL(Strategy(np), 
...
AMREX_CUDA_GLOBAL
void
PushParticlePositions (ParticlesData pData, amrex::Real dt)
{
...
AMREX_CUDA_GLOBAL
void
EnforcePeriodicBCs (ParticlesData pData, amrex::GeometryData geomData)
{
    int index, threadSize;
    getThreadIndex(index, threadSize, pData.size());
