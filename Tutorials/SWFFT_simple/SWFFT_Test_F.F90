...
contains
  ...
  subroutine fort_init_rhs (lo, hi, rhs, rlo, rhi, domlo, domhi, problo, probhi, dx, prob_type) &
    ...
    do k = lo(3), hi(3)
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             CASE (0)
                rhs(i,j,k) = -fac * sin(tpi*x)
             CASE (1)
                rhs(i,j,k) = -fac * (sin(tpi*x) * sin(tpi*y) * sin(tpi*z))  &
                     &       -fac * (sin(fpi*x) * sin(fpi*y) * sin(fpi*z))
             CASE (2)
                rhs(i,j,k) = -fac * (sin(1.d0*tpi*x) * sin(1.d0*tpi*y) * sin(1.d0*tpi*z))  &
             ...
             CASE (3)
                rhs(i,j,k) = exp(-5.d2*((x/Lx-0.5d0)**2 + (y/Ly-0.5d0)**2 + (z/Lz-0.5d0)**2))
             CASE DEFAULT
                rhs = 0.d0
             END SELECT
