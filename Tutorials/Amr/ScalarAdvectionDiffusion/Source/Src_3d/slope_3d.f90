module slope_module
  ...
  public :: slopex, slopey, slopez,slopex_nolim, slopey_nolim, slopez_nolim
...
contains
  ...
  subroutine slopex_nolim(lo, hi, &
    ...
    do    k = lo(3), hi(3)
       do j = lo(2), hi(2)
          ...
          ! Now unlimited fourth order slopes
          do i = lo(1), hi(1)
             dq1 = four3rd*dcen(i) - sixth*(df(i+1) + df(i-1))
             dq(i,j,k) = dq1
          end do
  ...
  subroutine slopez_doit_nolim(lo, hi, &
                         q, qlo, qhi, &
                         dq, dqlo, dqhi, &
                         dsgn, dlim, df, dcen, ddlo, ddhi)
  ...
  end subroutine slopez_doit_nolim
