...
contains
  ...
  subroutine my_make_new_level_from_scratch (lev, time, pba, pdm) bind(c)
   ...
   if (lev > 0) then
      call amrex_flash_fluxregister_build(flux_reg(lev), ba, amrex_get_boxarray(lev-1), &
           dm, amrex_get_distromap(lev-1), lev, ncomp)
   end if
  ...
  subroutine my_clear_level (lev) bind(c)
    ...
    call amrex_flash_fluxregister_destroy(flux_reg(lev))
