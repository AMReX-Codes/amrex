...
!     Compute hyperbolic fluxes using unsplit Godunov
      call umeth2d(time,q,uin_l1,uin_l2,uin_h1,uin_h2, &
...
! ::: ----------------------------------------------------------------
      ...
      subroutine umeth2d(time, q, qd_l1, qd_l2, qd_h1, qd_h2,&
                         srcQ, src_l1, src_l2, src_h1, src_h2, &
...
!     Local scalar variables
      double precision :: hdtdx, hdt, hdtdy, pi
      double precision :: div_u, div_max
      integer          :: i,j
      ...
      allocate ( psi(ilo1-2:ihi1+2,ilo2-2:ihi2+2) )
...
!     Local constants
      ...
      ! print *,'UGDX ',ugdx_l1, ugdx_h1, ugdx_l2, ugdx_h2
      ! print *,'UGDy ',ugdy_l1, ugdy_h1, ugdy_l2, ugdy_h2

      do j = ilo2-2, ihi2+2
      do i = ilo1-2, ihi1+2
         x = (dble(i)+0.5d0)*dx
         y = (dble(j)+0.5d0)*dy
         psi(i,j) =  sin(pi*x)**2 * sin(pi*y)**2 * cos (pi*time/2.d0) / pi
      end do
      end do
      ...
      do i = ugdx_l1,ugdx_h1
         ...
         ugdx(i,j) =  -( (psi(i,j+1)+psi(i-1,j+1)) - (psi(i,j-1)+psi(i-1,j-1)) ) / (4.d0*dy)
      end do
      end do
      ...
      do i = ugdy_l1,ugdy_h1
         ...
         ugdy(i,j) =  ( (psi(i+1,j)+psi(i+1,j-1)) - (psi(i-1,j)+psi(i-1,j-1)) ) / (4.d0*dx)
      end do
      end do
      ...
      do j = ilo2,ihi2
      do i = ilo1,ihi1
         div_u = (ugdx(i+1,j) - ugdx(i,j))/dx + (ugdy(i,j+1)-ugdy(i,j))/dy
         div_max = max(abs(div_u),div_max)
         if (i.eq.12.and.j.eq.0) print *,"DIVFLUX1 ", ugdx(i,j)-ugdx(i+1,j)
         if (i.eq.12.and.j.eq.0) print *,"DIVFLUX2 ", ugdy(i,j)-ugdy(i,j+1)
      end do
      end do
      print *,'ILO/IHI DIVMAX ',ilo1, ihi1, ilo2, ihi2, div_max
      ...
      do j = lo(2),hi(2)
         do i = lo(1),hi(1)
            uout(i,j,n) = uin(i,j,n) + dt * &
                 ...
                 +   dt * src(i,j,n)
               if (i.eq.12.and.j.eq.0) print *,"OLD RHO ",n, uin(i,j,URHO)
               if (i.eq.12.and.j.eq.0) print *,"NEW RHO ",n, uout(i,j,URHO)
               if (i.eq.12.and.j.eq.0) print *,"DIVFLUX1 ", flux1(i,j,n)-flux1(i+1,j,n)
               if (i.eq.12.and.j.eq.0) print *,"DIVFLUX2 ", flux2(i,j,n)-flux2(i,j+1,n)
         enddo
      enddo
         ...
         do j = lo(2),hi(2)
            do i = lo(1),hi(1)
               uout(i,j,n) = uin(i,j,n) + dt * &
                    ...
                    +   (flux2(i,j,n) - flux2(i,j+1,n)) / dx(2) ) &
                    +   dt * src(i,j,n)
               if (i.eq.12.and.j.eq.0) print *,"OLD SPEC OLD RHO ",n, uin(i,j,n), uin(i,j,URHO)
               if (i.eq.12.and.j.eq.0) print *,"NEW SPEC NEW RHO ",n, uout(i,j,n), uout(i,j,URHO)
            enddo
         enddo
      enddo
...
!  set min/max based on normal direction
      ...
      do j = jlo, jhi
         do i = ilo, ihi
            ...
            if (i.eq.12 .and. j.eq.0) print *,"QL QR RHO ",ql(i,j,QRHO), qr(i,j,QRHO)
