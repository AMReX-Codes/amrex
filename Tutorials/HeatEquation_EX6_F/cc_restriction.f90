...
contains
  ...
  subroutine lcc_restriction_1d(cc, loc, ff, lof, lo, hi, ir)
    integer,     intent(in)    :: loc(:)
    integer,     intent(in)    :: lof(:)
    integer,     intent(in)    :: lo(:), hi(:)
    logical, intent(inout) :: cc(loc(1):)
    logical, intent(in)    :: ff(lof(1):)
    integer,     intent(in)    :: ir(:)
    ...
    integer     :: i, twoi
       ...
       do i = lo(1), hi(1)
          ...
          cc(i) = ff(twoi) ! any of two
       end do

  end subroutine lcc_restriction_1d

  subroutine lcc_restriction_2d(cc, loc, ff, lof, lo, hi, ir)
    integer,     intent(in)    :: loc(:)
    integer,     intent(in)    :: lof(:)
    integer,     intent(in)    :: lo(:), hi(:)
    logical, intent(inout) :: cc(loc(1):,loc(2):)
    logical, intent(in)    :: ff(lof(1):,lof(2):)
    integer,     intent(in)    :: ir(:)
    ...
    if ( ir(1) == 2 .and. ir(2) == 2 ) then
       ...
       do j = lo(2), hi(2)
          ...
          do i = lo(1), hi(1)
             ...
             cc(i,j) = ff(twoi,twoj) ! any of four
          end do
       end do

    else
      write (*,*) 'r must be equal 2'
      stop
    end if

  end subroutine lcc_restriction_2d

  subroutine lcc_restriction_3d(cc, loc, ff, lof, lo, hi, ir)
    integer,     intent(in)    :: loc(:)
    integer,     intent(in)    :: lof(:)
    integer,     intent(in)    :: lo(:),hi(:)
    logical, intent(inout) :: cc(loc(1):,loc(2):,loc(3):)
    logical, intent(in)    :: ff(lof(1):,lof(2):,lof(3):)
    integer,     intent(in)    :: ir(:)
       ...
       do k = lo(3),hi(3)
          ...
          do j = lo(2),hi(2)
             ...
             do i = lo(1),hi(1)
                ...
                cc(i,j,k) =  ff(twoi,   twoj,   twok  ) ! any of eight
             end do
          end do
       end do
  ...
  end subroutine lcc_restriction_3d
