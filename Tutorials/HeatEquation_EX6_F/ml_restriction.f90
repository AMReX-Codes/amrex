...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine lml_cc_restriction_c(crse, cc, fine, cf, ir, nc)
    use cc_restriction_module
    type(lmultifab), intent(inout) :: fine
    type(lmultifab), intent(inout) :: crse
    ...
    do i = 1, nfabs(fine)
       ...
       do n = 1, lnc
          ...
          select case (dm)
          case (1)
             call lcc_restriction_1d(cp(:,1,1,1), lo, fp(:,1,1,1), lof, lo, hi, ir)
          case (2)
             call lcc_restriction_2d(cp(:,:,1,1), lo, fp(:,:,1,1), lof, lo, hi, ir)
          case (3)
             call lcc_restriction_3d(cp(:,:,:,1), lo, fp(:,:,:,1), lof, lo, hi, ir)
          end select
       end do
    end do
    !$OMP END PARALLEL DO
    ...
    call lmultifab_fill_boundary_c(crse,cc,lnc)

  end subroutine lml_cc_restriction_c

!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  subroutine lml_cc_restriction(crse, fine, ir)
    type(lmultifab), intent(inout) :: fine
    type(lmultifab), intent(inout) :: crse
    integer,        intent(in)    :: ir(:)
    if ( ncomp(crse) .ne. ncomp(fine) ) then
       call bl_error('lml_cc_restriction: crse & fine must have same # of components')
    end if
    call lml_cc_restriction_c(crse, 1, fine, 1, ir, ncomp(crse))
  end subroutine lml_cc_restriction
